 ...
 * @author Lukas Brandl
 */
public class MessagesDroppedTotal extends AbstractMetricEntry implements SYSTopicEntry {

    @Inject
    public MessagesDroppedTotal(final BlockingMetricService metricService) {
        super(metricService);
    }

    @Override
    public String topic() {
        return "$SYS/broker/messages/publish/dropped";
    }
    ...
    public String description() {
        return "The total number of publish messages that have been dropped due to inflight/queuing limits";
    }
    ...
    private class SysTopicSupplier implements Supplier<byte[]> {
        ...
        @Override
        public byte[] get() {
            final Counter metric = metricService.getHiveMQMetric(HiveMQMetrics.DROPPED_MESSAGE_COUNT);
