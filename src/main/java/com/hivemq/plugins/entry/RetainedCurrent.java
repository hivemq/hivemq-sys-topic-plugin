 ...
 * @author Lukas Brandl
 */
public class RetainedCurrent implements SYSTopicEntry {

    private final MetricService metricService;

    private final String topic = "$SYS/broker/messages/retained/count";

    @Inject
    public RetainedCurrent(final MetricService metricService) {
    ...
    private class SysTopicSupplier implements Supplier<byte[]> {
        ...
        @Override
        public byte[] get() {
            final Gauge<Integer> metric = metricService.getHiveMQMetric(HiveMQMetrics.RETAINED_MESSAGES_CURRENT);
            return Integer.toString(metric.getValue()).getBytes();
        }
    }
}
