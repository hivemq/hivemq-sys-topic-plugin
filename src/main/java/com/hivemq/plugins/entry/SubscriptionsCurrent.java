 ...
 * @author Lukas Brandl
 */
public class SubscriptionsCurrent extends AbstractMetricEntry implements SYSTopicEntry {

    @Inject
    protected SubscriptionsCurrent(BlockingMetricService metricService) {
        super(metricService);
    }
    ...
    private class SysTopicSupplier implements Supplier<byte[]> {
        ...
        @Override
        public byte[] get() {
            final Counter metric = metricService.getHiveMQMetric(HiveMQMetrics.SUBSCRIPTIONS_CURRENT);
